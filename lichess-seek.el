...
and preferred color."
  (interactive)
  (let* ((seek-type (completing-read "Seek Type: " '("Real-time" "Correspondence") nil t "Real-time"))
         (rated (y-or-n-p "Rated? "))
         (color (completing-read "Your Color: " '("random" "white" "black") nil t "random"))
         (variant (completing-read "Variant: " lichess-core-variants nil t "standard"))
         (rating-range (read-string "Rating Range (optional, e.g., 1500-1800): ")))
    (if (string= seek-type "Real-time")
        (let ((time (read-number "Time limit (minutes): " lichess-seek-default-time))
              (increment (read-number "Increment (seconds): " lichess-seek-default-increment)))
          (lichess-seek--real-time time increment rated color variant rating-range))
      (let ((days (completing-read "Days per turn: " '("1" "2" "3" "5" "7" "10" "14") nil t "1")))
        (lichess-seek--correspondence (string-to-number days) rated color variant rating-range)))))

(defun lichess-seek--real-time (time increment rated color variant rating-range)
...
and RATING-RANGE is optional opponent rating filter."
  (when lichess-seek--active-stream
    (when (y-or-n-p "An active seek is already running. Cancel it and start new one? ")
      (lichess-seek-cancel)))
  (unless lichess-seek--active-stream
    ...
    (let ((params `(("time" ,(number-to-string time))
                    ("increment" ,(number-to-string increment))
                    ("rated" ,(if rated "true" "false"))
                    ("variant" ,variant))))
      (unless (string= color "random")
        (push `("color" ,color) params))
      (when (and rating-range (not (string-empty-p rating-range)))
        (push `("ratingRange" ,rating-range) params))
      (message "Opening seek connection to Lichess...")
      (setq lichess-seek--active-stream
            (lichess-http-stream-open
             ...
             :on-open
             (lambda (_proc _buf)
               (message "Seeking game (%s %s+%s, rated: %s)... Press M-x lichess-seek-cancel to cancel."
                        variant
             ...
             (lambda (_proc _msg)
               (unless lichess-seek--cancelling
                 (message "Seek connection closed by Lichess. (Seek accepted or expired)"))
               (setq lichess-seek--active-stream nil)))))))

(defun lichess-seek--correspondence (days rated color variant rating-range)
...
and RATING-RANGE is optional opponent rating filter."
  ...
  (lichess-api-board-seek-correspondence
   ...
   variant
   color
   (if (string-empty-p rating-range) nil rating-range)
   (lambda (res)
     (if (lichess-http-result-success res)
         (let* ((data (lichess-http-result-data res))
                (id (lichess-util--aget data 'id)))
           (message "Correspondence seek created successfully! ID: %s. Wait for a player to join." id))
