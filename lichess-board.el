...
EVAL-STR: Optional evaluation string."
  (let ((s lichess-board-style))
    (if (and (string= s "gui")
      ...
      (let ((tui-style (if (string= s "gui") "unicode" s)))
        (lichess-board-tui-draw pos tui-style perspective eval-str)))))
...
(defun lichess-board-draw-heading (pos &optional perspective)
  ...
  (let* ((s lichess-board-style)
         (tui-style (if (string= s "gui") "unicode" s)))
